...
import { editRole, getModules } from "../../services/roles";
...
const EditRole = () => {
  ...
  useEffect(() => {
    ...
    if (allows) {
      setSelectedModules(allows);
    }
  }, [role, description, setValue, allows]);
  ...
  const onSubmit = async (data: CreateRoleParams) => {
    ...
    try {
      ...
      if (res.status === 200) {
        setLoading(false);
        showToast("success", res.data.message || "Role edited successfully");
        navigate("/roles");
      }
    } catch (error: any) {
      console.error(error);
