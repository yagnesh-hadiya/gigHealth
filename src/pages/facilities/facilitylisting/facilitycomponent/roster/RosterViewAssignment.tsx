...
const RosterViewAssignment = ({
  ...
  const fetch = useCallback(async () => {
    ...
    const [currentProfessional, currentJob, data] = await Promise.all([
      ...
      getJobDetails(row.FacilityId, row.JobId),
  ...
  const onSubmit = async (data: OfferFormType) => {
    ...
    try {
      ...
      const res = await RosterServices.editAssignment(editAssignmentDetails);
  ...
  return (
    <>
      ...
      <Modal
        ...
        onClosed={toggle}
      >
        <ModalHeader toggle={toggle}>View Offer</ModalHeader>
        <ModalBody
          className="RosterViewAssignment"
          style={{
            height: "800px",
            overflow: "auto",
                  ...
                  <div className="first-section-content">
                    ...
                    {reqId && (
                      <CustomInput
                        placeholder=""
                        disabled
                        value={`Req ID: ${reqId.toUpperCase()}`}
                        className="in-width input"
                        style={{
                          marginRight: "10px",
                        }}
...
export default RosterViewAssignment;
