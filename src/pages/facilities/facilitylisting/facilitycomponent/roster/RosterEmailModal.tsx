...
type RosterModalProps = {
  ...
  formData: OfferExtensionData;
  jobAssignmentId: number;
  toggleParent: () => void;
  toggleParentParent: () => void;
  fetchRosterData: () => void;
};

type RosterEmailModal = {
  subject: string;
};
...
const RosterEmailModal = ({
  ...
  useEffect(() => {
    setValue(
      "subject",
      `Gig Extension Request: ${capitalize(
        currentProfessional &&
        ...
        currentProfessional?.JobProfession
          ...
          : ""
      }, ${capitalize(job && job?.Title)} Required, Extension Start Date ${
        formData && formData?.startDate.replace(/-/g, "/")
      }`
    );
  }, [currentProfessional, formData, job, jobAssignment, setValue]);
 ...
 <p style="margin-bottom:20px;color:"red">The following healthcare professional would like to extend:</p>
   ...
   <p><strong>Extension Start Date:</strong> ${
     formData && moment(formData?.startDate).format("MM-DD-YYYY")
   }</p>
   <p><strong>Extension End Date:</strong> ${
     formData && moment(formData?.endDate).format("MM-DD-YYYY")
   }</p>
   <p><strong>Requested Time Off:</strong> ${
     formData &&
     formData?.approvedTimeOff?.map((date) => moment(date).format("MM-DD-YYYY"))
   }</p>
   <p><strong>Additional Notes:</strong>${
     jobAssignment?.Notes ? jobAssignment.Notes : ""
   }</li>
   <br /><br />
 <p>Thanks,</p>
...
`;
  ...
  const onSubmit = async (data: RosterEmailModal) => {
    ...
    //   ccEmails: selectedCcs.map((email: any) => email.value),
    //   bccEmails: selectedBccs.map((email: any) => email.value),
    ...
    setLoading("loading");
    try {
      const response = await RosterServices.requestExtension({
...
export default RosterEmailModal;
