...
const EditTemplate = () => {
  ...
  //     try {
  //         setLoading(true);
  //         const jobTemplate = await getListJobTemplate(Number(facilityDataId?.facilityId));
  //         dispatch({ type: JobActions.SetTemplate, payload: jobTemplate?.data?.data });
  //         setLoading(false);
  //     } catch (error: any) {
  //         console.error(error);
  //         setLoading(false);
  ...
  //   try {
  //     setLoading(true);
  //     const specialities = await getJobTemplateSpecialities(
  //       Number(facilityDataId?.facilityId)
  //     );
  //     dispatch({
  ...
  const getTemplateStatus = async () => {
    try {
      setLoading(true);
      const status = await getJobTemplateJobStatus(
        Number(facilityDataId?.facilityId)
      );
  ...
  useEffect(() => {
    ...
    const getTemplateComplianceDetails = async () => {
      try {
        ...
        const list = await getDocumentsList(Number(facilityDataId?.facilityId));
        dispatch({
  ...
  useEffect(() => {
    ...
    const getTemplate = async () => {
      try {
        ...
        setTotalHours(
          templateData?.HrsPerWeek + templateData?.OvertimeHrsPerWeek
        );
        setValue(
        ...
        setGrosspay(templateData?.GrossPay);
  ...
  useEffect(() => {
    const getTemplate = async () => {
      try {
        ...
        const template = await getJobTemplate(
          Number(facilityDataId?.facilityId),
          Number(checklistId?.Id)
        );
        const templateData = template?.data?.data[0];

        dispatch({
          type: JobActions.SetJobTemplate,
        ...
        setValue("pay.callBackRate", templateData?.CallBackRate);
        setValue("pay.overtimeRate", templateData?.OvertimeRate);
        // setValue("contract", capitalize(templateData?.Contract));
        setContent(templateData?.Description);
        // dispatch({
        //   type: JobActions.SetFeaturedJob,
        //   payload: templateData?.IsFeaturedJob,
  ...
  const getTemplateJobStatus = async () => {
    try {
      setLoading(true);
      const jobStatus = await getJobStatus(Number(facilityDataId?.facilityId));
      dispatch({
  ...
  const handleShiftStartTime = (
    ...
    //     newTime.setHours(Number(timeString[0]), Number(timeString[1]), Number(timeString[2] || 0));
    //     return newTime;
    // }

    // const currentTime = new Date();
    // const selectedTime = parseTimeString(selectedOption.value);

    // if (selectedTime >= currentTime) {
    //     dispatch({ type: JobActions.SetSelectedShiftStart, payload: { value: selectedOption?.value, label: selectedOption?.label } });

    // if (selectedShiftEnd && new Date(selectedShiftEnd?.value) <= selectedTime) {
    //     dispatch({ type: JobActions.SetSelectedShiftEnd, payload: null });
    //     return showToast('error', 'End time must be greater than start time');
    // } else {
    //     showToast('error', 'Selected time cannot be in past');
    // }

    // } else {
    //     dispatch({ type: JobActions.SetSelectedShiftStart, payload: null });
    //     return showToast('error', 'Selected time cannot be in past');
    // }
  };
  ...
  const handleCancel = () => {
    ...
    navigate(`/facility/${Number(facilityDataId?.facilityId)}`);
  };
  ...
  const onSubmit = async (data: JobType) => {
    ...
    try {
      setLoading(true);
      const template = await editJobTemplate({
        facilityId: Number(facilityDataId?.facilityId),
        templateId: Number(facilityDataId?.Id),
        title: data.title,
      ...
      showToast(
        "success",
        template?.data?.message || "Job Template Edited Successfully"
      );
      setLoading(false);
      setTimeout(() => {
        navigate(`/facility/${Number(facilityDataId?.facilityId)}`);
    ...
    <>
      <div className="navigate-wrapper d-flex justify-content-between">
        ...
        <div className="create-template-btn">
          <Link to={`/facility/${Number(facilityDataId?.facilityId)}`}>
                  ...
                  <Row>
                      ...
                      <CustomInput
                        ...
                        value={
                          totalHours
                            ...
                            : Number(watch("contractDetails.hrsPerWeek")) +
                              Number(
                                watch("contractDetails.overtimeHrsPerWeek")
                              )
                        }
                    ...
                    <Col md="6" lg="4" xl="3" className="col-group">
                      ...
                      <CustomInput
                        ...
                        value={grossPay ? grossPay : totalGrossPay?.toFixed(2)}
                        disabled
                      />
                    </Col>
                    <Col
