...
type OpeningsCandidateInformationProps = {
  toggle: () => void;
  facilityId: number;
  jobId: number;
...
const OpeningsCandidateInformation = ({
  toggle,
  facilityId,
  professionalId,
  jobApplicationId,
  ...
  row,
  isReadOnly,
}: OpeningsCandidateInformationProps) => {
  const [loading, setLoading] = useState<"loading" | "idle" | "error">("idle");

  const handleApproveExtension = async () => {
    setLoading("loading");
    try {
      const res = await RosterServices.approveExtension({
        facilityId,
        jobId,
        professionalId,
        jobApplicationId,
          ...
          <Row>
            ...
            <Col xxl="4" xl="4" lg="6" md="6" className="col-group">
              ...
              <CustomInput
                placeholder=""
                disabled
                value={row.ReqId ? row.ReqId.toUpperCase() : "-"}
              />
            </Col>
          </Row>
          {isReadOnly === false && (
...
export default OpeningsCandidateInformation;
